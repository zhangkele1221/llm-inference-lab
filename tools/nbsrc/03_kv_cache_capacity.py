...
# 3. **先扣开销，再扣权重**：80 GiB × 0.88 ≈ 70 GiB（那 12% 是留给激活值、CUDA graph、通信 buffer 和显存碎片的），再减掉权重 8B × 2 字节 ≈ 16 GB，剩下约 54 GiB 全部给 KV cache。
# 4. **算每请求 KV**：Llama-3-8B 每 token = `2 × 32层 × 8KV头 × 128 × 2字节` = **128 KiB**。所以 **4K 上下文是 0.5 GiB，8K 是 1 GiB**。
# 5. **相除得并发**：54 GiB ÷ 0.5 GiB ≈ **110 条**（4K）；如果上下文换成 8K，就只剩 **55 条**。**上下文翻倍，并发减半**——这句话比任何一个具体数字都值钱。
# 6. **补一句工程现实**：这是理论值，实际还要扣掉 block 内碎片（第 05 章）、prefill 峰值显存，以及调度上要留的余量，通常按 70-80% 打折规划。
#
# 第 5 步的"上下文翻倍并发减半"和第 6 步的工程打折，是拉开差距的地方——说明你做过真实部署，不是只会套公式。
#
# **作业**
#
